 ...
 * @notice known issue: the totalSupply() does not account for the interest accrued by users.
 * @notice known issue: the transfer() might re-set the interest rate of wallet.
 */
contract RebaseToken is ERC20, Ownable, AccessControl {
    ...
    //It's simply a unique bytes32 identifier that acts as a label for the role.
    ...
    //s_userInterestRate is stored as a per-second rate, not an annual rate.
    ...
    function principalBalanceOf(address _user) external view returns (uint256) {
        return super.balanceOf(_user); //calling straight to ERC20's original balanceOf
    }
    ...
    function transfer(
        ...
        // type(uint256).max means all tokens.
