...
contract CrossChainTest is Test {
    ...
    function setUp() public {
        ...
        sepoliaFork = vm.createSelectFork(
            vm.envString("sepolia_fork")
            //Create + Switch
        ); //it returns the forkId
        arbSepoliaFork = vm.createFork(
            vm.envString("arbSepoliaFork")
            //create for later
        );
        ...
        //vm.makePersistent() makes a contract or address persist across fork switches.
        //Without it, contracts deployed on one fork disappear when you switch to another fork.
        ...
        //It points to a real contract already deployed by Chainlink on Sepolia. The fork copies all existing state.

        registryModuleOwnerCustomSepolia.registerAdminViaOwner(
            address(sourceRebaseToken) // msg.sender becomes the admin of sourceRebaseToken
        );
        //"I (msg.sender) want to register as admin FOR this token (sourceRebaseToken)"
        // msg.sender here is owner

        // msg.sender accept role on Sepolia
    ...
    function configureTokenPool(
        ...
        vm.selectFork(fork);
        vm.startPrank(owner);
        TokenPool.ChainUpdate[] memory chains = new TokenPool.ChainUpdate[](1); //Creates a new array with length 1
        ...
        uint64[] memory remoteChainSelectorsToRemove = new uint64[](0); //an empty array with length zero.
    ...
    function testBridgeTwice() public {
        ...
        // Deposit to the vault and receive tokens
        Vault(payable(address(vault))).deposit{value: SEND_VALUE}();
        uint256 sourceUserInterestRate = sourceRebaseToken.getUserInterestRate(
            alice
        );
        uint256 startBalance = IERC20(address(sourceRebaseToken)).balanceOf(
            alice
        ...
        uint256 destBalance = IERC20(address(destRebaseToken)).balanceOf(alice);
        uint256 destUserInterestRate = destRebaseToken.getUserInterestRate(
            alice
        );
        assertEq(sourceUserInterestRate, destUserInterestRate);
        console.log("Amount bridging back %d tokens ", destBalance);
        bridgeTokens(
            destBalance,
            arbSepoliaFork,
